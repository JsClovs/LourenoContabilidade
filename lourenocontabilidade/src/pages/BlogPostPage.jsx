...
function BlogPostPage() {
  ...
  if (!post) {
     return (
        <div className="text-center py-10">
            ...
            <p className="text-gray-700 mb-6">Desculpe, o post com o slug "{slug}" não existe ou foi removido.</p>
            <Link
  ...
  return (
    <article className="prose lg:prose-xl max-w-none mx-auto p-4">
      <Link to="/blog" >
             ← Voltar
        </Link>
      <h1 className="text-4xl font-extrabold mb-3">{post.title}</h1>
      <p className="text-md text-gray-500 mb-8 -mt-2">
        Por {post.author} em {post.date ? new Date(post.date).toLocaleDateString() : 'Data inválida'}
      </p>
